...
// ✅ Cho phép mọi site Netlify & mọi domain *.ngrok-free.app
...
const corsDelegate = {
  origin(origin, cb) {
    ...
    if (netlifyRegex.test(origin)) return cb(null, true);   // ⬅️ Netlify
    if (ngrokRegex.test(origin)) return cb(null, true);     // ⬅️ Ngrok
...
const io = new Server(server, {                                                     // ⬅️ thêm
  cors: {
    origin: (origin, cb) => {
      ...
      if (localhostRegex.test(origin)) return cb(null, true);
      if (lanRegex.test(origin)) return cb(null, true);
      if (netlifyRegex.test(origin)) return cb(null, true);   // ⬅️ Netlify cho Socket.IO
      if (ngrokRegex.test(origin)) return cb(null, true);     // ⬅️ Ngrok cho Socket.IO
